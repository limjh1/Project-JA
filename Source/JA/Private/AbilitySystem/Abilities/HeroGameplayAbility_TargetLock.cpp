...
void UHeroGameplayAbility_TargetLock::OnTargetLockTick(float DeltaTime)
{
	if (!CurrentLockedActor ||
		UJAFunctionLibrary::NativeDoesActorHaveTag(CurrentLockedActor, JAGameplayTags::Shared_Status_Dead) ||
		UJAFunctionLibrary::NativeDoesActorHaveTag(GetHeroCharacterFromActorInfo(), JAGameplayTags::Shared_Status_Dead)
	)
	{
	...
	const bool bShouldOverrideRotation =
		(!UJAFunctionLibrary::NativeDoesActorHaveTag(GetHeroCharacterFromActorInfo(), JAGameplayTags::Player_Status_Rolling)) &&
		(!UJAFunctionLibrary::NativeDoesActorHaveTag(GetHeroCharacterFromActorInfo(), JAGameplayTags::Player_Status_Blocking));
...
void UHeroGameplayAbility_TargetLock::SwitchTarget(const FGameplayTag& InSwitchDirectionTag)
{
	...
	if (InSwitchDirectionTag == JAGameplayTags::Player_Event_SwitchTarget_Left)
	{
		NewTargetToLock = GetNearestTargetFromAvailableActors(ActorsOnLeft);
	}
	else if (InSwitchDirectionTag == JAGameplayTags::Player_Event_SwitchTarget_Right)
	{
	...
	if (NewTargetToLock)
	{
		CurrentLockedActor = NewTargetToLock; // Tick 함수에서 갱신
	}
}
...
void UHeroGameplayAbility_TargetLock::DrawTargetLockWidget()
{
	if (!DrawnTargetLockWidget)
	{
		...
		DrawnTargetLockWidget = CreateWidget<UJAWidgetBase>(GetHeroControllerFromActorInfo(), TargetLockWidgetClass);
...
{
	if (0.f < CachedDefaultMaxWalkSpeed)
	{
